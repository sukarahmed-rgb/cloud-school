...
export function setupKeyboardShortcuts() {
    document.addEventListener('keydown', function(e) {
        ...
        if (!ctrl) {
            switch (key) {
                ...
                case 'r': case 'R':
                    ...
                    if (roleBar) {
                        ...
                        window.speak(roleBar.classList.contains('hidden') ? window.__('roleBarHidden') : window.__('roleBarVisible'));
                    }
                    break;
            }
        } else {
            ...
            switch (key) {
                case 'm': case 'M': e.preventDefault(); window.toggleAudioRecording(); break;
                case 's': case 'S': if (shift) { e.preventDefault(); window.toggleScreenReaderMode(); } break;
                case 'a': case 'A': if (shift) { e.preventDefault(); window.toggleAudioCoPilot(); } break;
                case 't': case 'T': if (shift) { e.preventDefault(); window.toggleTtsEngine(); } break;
                case '=': e.preventDefault(); window.adjustTextSize(1); break;
                case '-': e.preventDefault(); window.adjustTextSize(-1); break;
            }
        }
    });
