...
export function renderStudentStats() {
  ...
  const avgScore =
    quizCount > 0
      ? Math.round(mySubs.reduce((sum, s) => sum + (s.initialScore || 0), 0) / quizCount)
      : null;
...
export function renderStudentDashboard() {
  ...
  if (mySubs.length === 0) {
    ...
    const avg = Math.round(
      mySubs.reduce((sum, s) => sum + (s.initialScore || 0), 0) / mySubs.length,
    );
    const last = mySubs[0];
    quizDiv.innerHTML =
      '<div class="space-y-2">' +
  ...
  if (
    mySubs.reduce((sum, s) => sum + (s.initialScore || 0), 0) / Math.max(mySubs.length, 1) >=
    80
  ) {
  ...
  achDiv.innerHTML = `<div class="space-y-2">${badges
    .map((b) => `<p class="text-white font-bold text-lg">${b}</p>`)
    .join('')}</div>`;
