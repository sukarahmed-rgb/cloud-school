...
export let activeGameType = '';
export let currentGameScore = 0;
export let gameTimeLeft = 30;
export let gameTimerInterval = null;
export let currentCorrectAnswer = null;

export let audioMemorySequence = [];
export let audioMemoryStep = 0;
...
export let currentMathAnswer = null;
export let currentAIGameData = null;
...
function checkMathAnswer() {
  ...
  if (
    userAns === String(currentMathAnswer) ||
    userAns.replace(/\s/g, '') === String(currentMathAnswer).replace(/\s/g, '')
  ) {
    const level = getAgeLevelDifficulty();
...
function showListeningQuestion() {
  ...
  const qData = currentAIGameData.questions[currentAIGameData.currentQ];
...
function showScienceStep() {
  ...
  const step = currentAIGameData.steps[currentAIGameData.currentStep];
  const stepText = `${window.__('gameScienceStep', currentAIGameData.currentStep + 1)}: ${step.description}`;
...
export function startNewGameRound() {
  ...
  if (activeGameType === 'hero') {
    if (currentGameScore >= 80) {
      level = 3;
    } else if (currentGameScore >= 40) {
      level = 2;
    }
  }
...
export function endGame() {
  ...
  if (currentGameScore >= 50) {
    window.addNotification(
      ...
      `${window.currentUserSession?.name || window.__('notifStudent')} ${window.__(
        'notifGotScore',
      )} ${currentGameScore} ${window.__('notifPointsInGame')} ${
        activeGameType === 'audio-memory'
          ? window.__('notifMemoryGame')
          : window.__('notifQuizGame')
      }!`,
      'achievement',
    );
  }
...
export function playAudioMemorySequence() {
  ...
  function playNext() {
    ...
    const pattern = audioMemoryPatterns[audioMemorySequence[i]];
